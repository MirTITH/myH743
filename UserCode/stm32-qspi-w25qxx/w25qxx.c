...
W25_result_t w25_init()
{
    ...
    if (HAL_QSPI_Receive(&hqspi, rx_buffer, HAL_QPSI_TIMEOUT_DEFAULT_VALUE) != HAL_OK) {
        Error_Handler();
    }
...
W25_result_t w25_read(uint32_t address, uint8_t *dat, uint32_t len)
{
    ...
    // Execute QSPI reception
    if (HAL_QSPI_Receive(&hqspi, dat, HAL_QSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK) {
        Error_Handler();
    }
...
W25_result_t w25_write(uint32_t address, uint8_t *dat, uint32_t len)
{
    ...
    for (uint32_t page = start_page; page <= end_page; page++) {
        ...
        if (HAL_QSPI_Transmit(&hqspi, &dat[data_start], HAL_QSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK) {
            Error_Handler();
        }
